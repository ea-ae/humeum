...
public record DeleteAssetCommand : ICommand<IResult<None>> {
    [Required] public required int Profile { get; init; }
    [Required] public required int Asset { get; init; }
}

public class DeleteAssetCommandHandler : ICommandHandler<DeleteAssetCommand, IResult<None>> {
    readonly IAppDbContext _context;

    public DeleteAssetCommandHandler(IAppDbContext context) => _context = context;

    public async Task<IResult<None>> Handle(DeleteAssetCommand request, CancellationToken token = default) {
